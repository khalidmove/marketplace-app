        ...
        <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
          {productdata?.price_slot &&
            ...
            productdata.price_slot.map((item,i) => (
              <TouchableOpacity style={[styles.box,{marginRight:productdata?.price_slot.length===i+1?20:10,backgroundColor:selectedslot?.value===item.value?Constants.lightpink:Constants.customgrey3,borderColor:selectedslot?.value===item.value?Constants.linearcolor:Constants.customgrey}]} key={i} onPress={()=>setsselectedslot(item)}>
                <ImageBackground
      ...
      {currentproduct && (
        <TouchableOpacity
          ...
          <Text style={styles.buttontxt}>
            {' '}
            {cartdetail.length} {t('items')} | {Currency}{sumdata}
          </Text>
